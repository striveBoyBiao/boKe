...
<article class="blogs">
    ...
    <div class="index_about">
        ...
        <p class="box_c"><span class="d_time">发布时间：${list.gtmcreate}</span><span>编辑：<a href="mailto:1169763240@qq.com">${list.author}</a></span><span>阅读（<script src="./慢生活- 柴米油盐的生活才是真实 - 一个站在web前端设计之路的女技术员个人博客网站_files/saved_resource"></script>${list.reading}）</span></p>
        <ul class="infos">
            ${list.content}
        ...
        <div class="nextinfo">
            <p>上一篇：
                <a href='/main/findLearnDetails.do?cid=${ondate.cid}' title='${ondate.title}' >${ondate.title}</a>
            </p>
            <p>下一篇：
                <a href='/main/findLearnDetails.do?cid=${underDate.cid}' title='${underDate.title}' >${underDate.title}</a>
            </p>
        </div>
        <div class="otherlink">
            <h2>相关文章</h2>
            <ul>
                <c:forEach items="${relatedate}" var="relate" >
                    <li>
                        <a href='/main/findLearnDetails.do?cid=${rank.cid}' title='${relate.title}' >${relate.title}</a>
                    </li>
                </c:forEach>
            </ul>
        </div>
        ...
        <div class="news">
            ...
            <ul class="rank" id="lifedetailsnewsdate">
                  <c:forEach items="${newsdate}" var="news" >
                        <li>
                            <a href='/main/findLearnDetails.do?cid=${news.cid}' title='${news.title}' >${news.title}</a>
                        </li>
                  </c:forEach>
            </ul>
            <h3 class="ph">
            ...
            <ul class="paih" id="lifedetailsrankdate">
                <c:forEach items="${rankdate}" var="rank" >
                    <li>
                        <a href='/main/findLearnDetails.do?cid=${rank.cid}' title='${rank.title}' >${rank.title}</a>
                    </li>
                </c:forEach>
            </ul>
        </div>
